...
CREATE POLICY "Users can create teams" ON teams FOR INSERT WITH CHECK (auth.uid()::text = creator_id::text);
CREATE POLICY "Team creators can update their teams" ON teams FOR UPDATE USING (auth.uid()::text = creator_id::text);
CREATE POLICY "Team creators can delete their teams" ON teams FOR DELETE USING (auth.uid()::text = creator_id::text);
...
CREATE POLICY "Team creators can add members" ON team_members FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM teams WHERE id = team_id AND creator_id::text = auth.uid()::text)
);
CREATE POLICY "Users can leave teams" ON team_members FOR DELETE USING (user_id::text = auth.uid()::text);

-- Políticas para convites
CREATE POLICY "Users can view their invitations" ON team_invitations FOR SELECT USING (user_id::text = auth.uid()::text);
CREATE POLICY "Team creators can create invitations" ON team_invitations FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM teams WHERE id = team_id AND creator_id::text = auth.uid()::text)
);
CREATE POLICY "Users can update their invitations" ON team_invitations FOR UPDATE USING (user_id::text = auth.uid()::text);

-- Políticas para propriedades
CREATE POLICY "Users can view all properties" ON properties FOR SELECT USING (true);
CREATE POLICY "Users can create properties" ON properties FOR INSERT WITH CHECK (user_id::text = auth.uid()::text);
CREATE POLICY "Users can update their properties" ON properties FOR UPDATE USING (user_id::text = auth.uid()::text);
CREATE POLICY "Users can delete their properties" ON properties FOR DELETE USING (user_id::text = auth.uid()::text);
...
CREATE POLICY "Users can create messages" ON messages FOR INSERT WITH CHECK (sender_id::text = auth.uid()::text);
CREATE POLICY "Users can update their received messages" ON messages FOR UPDATE USING (receiver_id::text = auth.uid()::text);
