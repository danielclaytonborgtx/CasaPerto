...
const EditTeam: React.FC = () => {
  ...
  const fetchTeam = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`https://server-2-production.up.railway.app/team/${id}`);
  ...
  const handleUpdateTeam = async () => {
    ...
    if (teamName.trim() !== '' && brokers.length > 0) {
      ...
      try {
        await axios.put(`https://server-2-production.up.railway.app/team/${id}`, formData, {
  ...
  const handleDeleteTeam = async () => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`https://server-2-production.up.railway.app/team/${id}`);
    ...
    <Container>
      {teamImage ? (
          <div style={{ position: "relative", display: "inline-block" }}>
            <TeamImage 
              src={teamImage.startsWith("data:image") ? teamImage : `https://server-2-production.up.railway.app${teamImage}`} 
